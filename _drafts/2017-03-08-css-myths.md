...
Progressive enhancement promotes the idea of adding extra functionality for modern browsers and devices, only after delivering core content and functionality to all bowsers and devices. This differs slightly from graceful degradation which focuses on delivering the all singing all dancing solution first and then provides fallbacks for older browsers. 

In scripting languages both of these scenarios can be handled handled with feature detection. CSS can also provide feature detection without the need to use a plugin or external script such as Modernizr or Autoprefixer.

The @supports rule is a conditional rule which allows for different styles to be applied depending on browser support.

```
```

CODE EXAMPLE FOR NOT SUPPORTS - a nice to have anamate in thing

```
```

CODE EXAMPLE FOR NOT SUPPORTS

link https://www.lottejackson.com/learning/supports-will-change-your-life
