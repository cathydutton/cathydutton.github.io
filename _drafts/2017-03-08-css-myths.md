...
To work effectively a detailed understanding of key CSS features is essential. A misunderstanding of these features can often lead to out of control CSS.


#### Cascade
The cascade determines which properties will be applied, and can be thought of as a way of resolving conflicts between properties.

Which selector wins out in the cascade depends on three factors:

 * Importance - There are 3 different origins or types of style sheet: author, user, and user agent.

  by default, rules in author style sheets have more importance than rules in user style sheets. This is reversed, for rules using "!important". All rules user and author rules have more weight than rules in the UA's default style sheet.
 
 * Specificity - If the importance and origin is the same the decleration with the mst specificity will win out.
 ...
 * Source order - Declarations at the bottom of a file will effectively overwrite the ones at the top.
