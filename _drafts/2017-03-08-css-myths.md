...
```
The specificity of the selector above is just 0,0,1,0 and so the value could be easily overwritten further down the style sheet. To address this and make helper classes more resilient the !important rule should be applied.
...
It is also possible to detect support for a property within CSS without the need to include a third-party script such as Modernizr or Autoprefixer. The @supports rule is a conditional rule which allows for different styles to be applied depending on browser support. 
...
A bad workmen blames his tools, the featured of CSS are not secret, anyone can read up on and learn to write good CSS if you choose not to, CSS can become a nightmare.
